...
QUndoStack* QHexEdit::undoStack() const
{
    return qHexEdit_p->undoStack();
}
...
void QHexEdit::setInsertAllowed(bool insertAllowed)
{
    qHexEdit_p->setInsertAllowed(insertAllowed);
}

bool QHexEdit::isInsertAllowed()
{
    return qHexEdit_p->isInsertAllowed();
}
