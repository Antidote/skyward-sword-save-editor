...
QHexEditPrivate::QHexEditPrivate(QScrollArea *parent) : QWidget(parent)
{
    ...
    connect(_undoStack, SIGNAL(indexChanged(int)), this, SLOT(undoStackChanged()));
...
void QHexEditPrivate::setInsertAllowed(bool insertAllowed)
{
    _insertAllowed = insertAllowed;
}

bool QHexEditPrivate::isInsertAllowed()
{
...
QUndoStack* QHexEditPrivate::undoStack() const
{
    return _undoStack;
}
    ...
    /* Hex input */
        ...
        if ((key>='0' && key<='9') || (key>='a' && key <= 'f'))
        {
            ...
            if (_xData.size() > 0)
            {
                ...
                replace(posBa, QByteArray().fromHex(hexValue));
                setCursorPos(_cursorPosition + 1);
                resetSelection(_cursorPosition);
            }
        }
...
void QHexEditPrivate::undoStackChanged()
{
    update();
    adjust();
    ensureVisible();
